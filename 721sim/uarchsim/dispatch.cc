...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if(enable) {
         printf("\n%X We are in the DISPATCH enable\n", PAY.buf[index].pc);
         if(PAY.buf[index].good_instruction && PAY.buf[index].C_valid && !branch_flag && vp_perf) {
            ...
            printf("\n%X We are in the DISPATCH perfect if statement\n", PAY.buf[index].pc);
         }
         else if(eligible && !PAY.buf[index].miss && vpq_size > 0) {
            printf("\n%X We are in the DISPATCH eligible AND SVP if statement\n", PAY.buf[index].pc);
            if(PAY.buf[index].confident){
               REN->set_ready(PAY.buf[index].C_phys_reg);
               REN->write(PAY.buf[index].C_phys_reg, PAY.buf[index].prediction.dw);
               printf("\n%X We are in the DISPATCH eligible AND confident for SVP if statement\n", PAY.buf[index].pc);
            }
         }
      }
