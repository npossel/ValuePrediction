...
static void help()
{
  ...
  fprintf(stderr, "  --vp-enable=<n>    Set to enable value prediction\n");
  fprintf(stderr, "  --vp-perf=<n>      Set to spsecify perfect value prediction\n");
  fprintf(stderr, "  --vp-svp=<VPQsize>,<oracleconf>,<index>,<tag>,<confmax>,<confinc>,<confdec>,<replace_stride>,<replace>,<predINTALU>,<predFPALU>,<predLOAD>,<VPQ_full_policy>\tCheck document for more info.\n");
...
static void set_vp_flags(const char* config) {
   uint64_t VPQsize,oracleconf,index,tag,confmax,confinc,confdec,replace_stride,replace,predINTALU,predFPALU,predLOAD,VPQ_full_policy;
   if (sscanf(config, "%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu", &VPQsize, &oracleconf, &index, &tag, &confmax, &confinc, &confdec, &replace_stride, &replace, &predINTALU, &predFPALU, &predLOAD, &VPQ_full_policy) != 13) {
      fprintf(stderr, "Incorrect usage of --vp-svp=<VPQsize>,<oracleconf>,<index>,<tag>,<confmax>,<confinc>,<confdec>,<replace_stride>,<replace>,<predINTALU>,<predFPALU>,<predLOAD>,<VPQ_full_policy>");
      exit(-1);
   }
   else {
      ...
      VPQ_FULL_POLICY = (VPQ_full_policy ? true : false);
   }
}
...
int main(int argc, char** argv)
{
  ...
  parser.option(0, "vp-enable", 1, [&](const char* s){VP_ENABLE = atoi(s);});
  parser.option(0, "vp-perf", 1, [&](const char* s){VP_PERF = atoi(s);});
